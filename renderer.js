...
function browseResult(e){
    ...
    require('child_process').exec(`explorer.exe "${path}"`);
    path = fileselector.value;
    console.log(fileselector.value);
}

function openServerProperties(){
    const fs = require('fs')
    fs.readFile(path + 'server.properties', 'utf-8', (err, data) => {
    if (err) throw err;
    console.log(data);
    document.getElementById("management") = data;
    })
}

function downloadServerFile() {
    var forkValue = document.getElementById("fork").value;
    var verValue = document.getElementById("versions").value;
